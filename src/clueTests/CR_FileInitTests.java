...
public class CR_FileInitTests {
	...
	public void FourDoorDirections() {
		...
		RoomCell room = board.getRoomCell(4, 3);
		assertTrue(room.isDoorway());
		assertEquals(RoomCell.DoorDirection.RIGHT, room.getDoorDirection());
		room = board.getRoomCell(4, 8);
		assertTrue(room.isDoorway());
		assertEquals(RoomCell.DoorDirection.DOWN, room.getDoorDirection());
		room = board.getRoomCell(15, 18);
		assertTrue(room.isDoorway());
		assertEquals(RoomCell.DoorDirection.LEFT, room.getDoorDirection());
		room = board.getRoomCell(14, 11);
		assertTrue(room.isDoorway());
		assertEquals(RoomCell.DoorDirection.UP, room.getDoorDirection());
		// Test that room pieces that aren't doors know it
		room = board.getRoomCell(14, 14);
		assertFalse(room.isDoorway());	
		// Test that walkways are not doors
		BoardCell cell = board.getBoardCell(0, 6);
	...
	public void testRoomInitials() {
		assertEquals('C', board.getRoomCell(0, 0).getInitial());
		assertEquals('R', board.getRoomCell(4, 8).getInitial());
		assertEquals('B', board.getRoomCell(9, 0).getInitial());
		assertEquals('O', board.getRoomCell(21, 22).getInitial());
		assertEquals('K', board.getRoomCell(21, 0).getInitial());
	}
	...
	public void testBadColumns() throws BadConfigFormatException, FileNotFoundException {
		...
		ClueGame game = new ClueGame("ClueLayoutBadColumnsRader.csv", "ClueLegendRader.txt");
	...
	public void testBadRoom() throws BadConfigFormatException, FileNotFoundException {
		...
		ClueGame game = new ClueGame("ClueLayoutBadRoomRader.csv", "ClueLegendRader.txt");
		game.loadConfigFiles();
	}
	// Test that an exception is thrown for a bad config file
	@Test (expected = BadConfigFormatException.class)
